...
const char * const BITCOINR_CONF_FILENAME = "bitcoinr.conf";
const char * const BITCOINR_PID_FILENAME = "bitcoinrd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitcoinR
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitcoinR
    // Mac: ~/Library/Application Support/bitcoinR
    // Unix: ~/.bitcoinr
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitcoinR";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bitcoinR";
#else
    // Unix
    return pathRet / ".bitcoinr";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINR_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINR_PID_FILENAME)));
}
