...
###   bitcoinrcore.org and bitcoinr.org and compares them.
...
WORKINGDIR="/tmp/bitcoinr_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrcore.org"
HOST2="https://bitcoinr.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinr-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinr-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinr.org failed to provide signature file, but bitcoinrcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinr.org and bitcoinrcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the bitcoinR Core binary release signing key installed?"
   fi
